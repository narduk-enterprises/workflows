...
| `red-main-listener.yml` | **New 2026-09-24** (narduk-reboot P3-C2 / O-D8). Opens or refreshes ONE "main is red: `<workflow-name>`" issue, labelled `red-main`, per repo per listened workflow, the first time that workflow goes red on the default branch; closes it on the next green run. Optional fail-open portal-mirror POST hook. No automatic revert or rollback — that stays in the app's own deploy tooling |
